...
function SignupWidget({ redirect = true, signupActionPopup, changeContent }) {
  ...
  const handleDemoRedirect = () => {
    toast.info("This is a demo version. Redirecting to login page...");
    setTimeout(() => {
      if (redirect) {
        router.push('/login');
      } else {
  ...
  const doSignup = async () => {
    ...
    try {
      ...
      // Handle demo-related errors
      if (err?.response?.data?.message?.toLowerCase().includes('demo') ||
        err?.response?.status === 403 ||
        err?.response?.data?.demo === true) {
        handleDemoRedirect();
        return;
      }
